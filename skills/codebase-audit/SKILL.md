...
name: codebase-audit
description: >-
  在对现有前端代码库进行重构、添加新功能、排查复杂样式冲突或评估组件依赖时触发此 Skill。
---

# 前端代码库审计 (Frontend Codebase Audit)

本 Skill 对应 `Frank_Agentic_Workflow.md` 流程图中的 **1. 入口与分流层 -> 源码调研/代码库审计 (reportSource / codebase-audit-summary)** 节点。

## 目标 (Goal)
快速梳理前端项目现有的组件树结构、全局样式 Token 定义、依赖库版本及全局状态分布，避免无意义的重复轮子和样式污染。
...
```markdown
### 🔍 前端代码库审计摘要 (Codebase Audit Summary)
- **前端技术栈**：[例如 Vite + React + Vanilla CSS]
